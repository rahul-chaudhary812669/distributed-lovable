...
public class GatewayJwtAuthFilter implements GlobalFilter, Ordered {
      ...
      ServerHttpRequest request =  exchange.getRequest();
       ...
       String authHeader = request.getHeaders().getFirst("Authorization");
           if(authHeader==null && !authHeader.startsWith("Bearer ")){
               ...
               return  sendErrorResponse(exchange, HttpStatus.UNAUTHORIZED , "Missing or invalid Authorization header for path: {}"+ path);
           }
        ...
        String token =  authHeader.substring(7);
           ...
           try{
               ...
               log.error(" Jwt token validation failed in ApiGateway for path :{}", path);
    ...
    private Mono<Void> sendErrorResponse(ServerWebExchange exchange , HttpStatus status , String message) {
             ...
             try{
                 byte[] bytes = objectMapper.writeValueAsBytes(apiError); // object -> JSON -> [] bytes
