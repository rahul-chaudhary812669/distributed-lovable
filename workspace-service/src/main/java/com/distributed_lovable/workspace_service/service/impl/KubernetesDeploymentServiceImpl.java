...
public class KubernetesDeploymentServiceImpl implements DeploymentService {
    ...
    Pod findActivePod(Long projectId){
        return client.pods().inNamespace(namespace)
                ...
                .filter(pod -> pod.getStatus().getPhase().equals("Running"))  // because pod may be not working due to some issue
                .findFirst()
                .orElse(null);
    }
    ...
    private DeployResponse claimAndStartNewPod(Long projectId, String domain , String formattedUrl) {
        Pod pod =      client.pods().inNamespace(namespace)
                       ...
                       .stream()
                       .findFirst()
                       .orElseThrow(()-> new RuntimeException("No idle runners available , please scaleup the runner pool"));

        String podName = pod.getMetadata().getName();
        log.info(" claiming pod {} for project {} ", podName, projectId);

        client.pods().inNamespace(namespace)
                .withName(podName).edit(p -> {
        ...
        try {
            ...
            log.info("starting development server for project {}", projectId);
...
//                 }).exec(command)){
//
//             // wait briefly to ensure command fired (fabric8 is async)
//             // for long-running  background jobs , we don't wait for done
    ...
    private void execCommand(String podName, String container, String... command) {
        ...
        try (ExecWatch watch = client.pods()
                ...
                .usingListener(new ExecListener() {
                    ...
                    @Override
                    public void onOpen() {
                        log.debug("Exec session opened.");
                    }

                    @Override
                    public void onClose(int code, String reason) {
                        log.info("Command closed. ExitCode={}, Reason={}", code, reason);
            ...
            // If running a background command (ends with '&'),
            // just give it a moment to start and return.
            if (command.length > 0 &&
                    command[command.length - 1].trim().endsWith("&")) {
            ...
            log.info("Exec session closed with code {}", exitCode);
            ...
            log.error("Exec failed in {}:{} -> {}",
                    podName, container, String.join(" ", command), e);
            throw new RuntimeException("Pod execution failed", e);
        }
    }
